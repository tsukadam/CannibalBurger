...
public class CustomerController : MonoBehaviour
{
    ...
    public void TestMake() {
        ...
        while (count < 1)
        {
            MakeCustomer("test", "doc", 1, 0, 0, 2, new string[3], new string[3], 0);
            count++;
        }
    }

    //客生成
    public void MakeCustomer(string Name,string Image, int Hp,int CoreColor,int Color, int DropG,string[] DropItem,string[]DropMeat,int SaveSus)
    {
...
//        Customer.transform.parent = CustomerField.transform;
        ...
        //色決定
        int ColorR = Random.Range(30, 255);
        int ColorG = Random.Range(30, 255);
        int ColorB = Random.Range(30, 255);
...
//アイテム選択時用のボタンを不活性にする
        ...
        Customer.GetComponent<StatCustomer>().Color = Color;
 ...
 //        yield return new WaitForSeconds(1f);//遅延時間
     ...
     IEnumerator Make12Coroutine()
     {
         ...
         while (CountColumn <= 4)
         {
             ...
             while (CountRow <= 3)
             {
                 ...
                 int ClearCount = StatGame.GetComponent<StatGame>().ClearCustomer;
     ...
     {
             Debug.Log("Check");
         if (StatGame.GetComponent<StatGame>().NowBombCount > 0)
         {
     ...
     {
         StatGame.GetComponent<StatGame>().NowBombCount--;
     ...
     {
         GameObject Customer = GetObjectFromPosition(1, Column);
         int Damage= StatGame.GetComponent<StatGame>().StatBurgerPower;
         //FEED1のときは単独ボーナス
     ...
     IEnumerator LvUpCoroutine()
     {
         ...
         GetComponent<PlayerController>().DrawLvPower(PartName, "Power", StatGame.GetComponent<StatGame>().ClearCustomer/10);
 ...
 //        PopupText1.GetComponent<Text>().text=Text;
     ...
     IEnumerator CustomerDestroyCoroutine(GameObject Customer)
     {
         ...
         StatGame.GetComponent<StatGame>().ClearCustomer++;
         int Clear = StatGame.GetComponent<StatGame>().ClearCustomer;
         if (Clear %10 == 0)
         {
